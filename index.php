...
function renderTxRecent() {
  ...
  const recent = state.transactions.slice(0, RECENT_LIMIT);
  if (!recent.length) {
...
function updateAmtDisplay() {
  ...
  const na=document.getElementById('nativeAmt'); if(na)na.value=amountRaw||'';
}
...
function renderWalletSelectors() {
  const src = state.wallets.length ? state.wallets : WALLETS;
  const make = (cid, hid, def=0) => {
    const c=document.getElementById(cid); if(!c)return;
    c.innerHTML = src.map((w,i)=>{
    ...
    if(src[def])document.getElementById(hid).value=src[def].id;
  };
  make('fromWalletSel','selectedFromWallet',0);
  make('toWalletSel','selectedToWallet',src.length>1?1:0);
}
...
// Check sessionStorage for auto-open
if(sessionStorage.getItem('openAdd')==='1'){sessionStorage.removeItem('openAdd');setTimeout(openAddModal,300);}
